...
End your run by emitting **exactly one** JSON object, and nothing else that competes to be the result. The object is keyed by the input Item ids; each value is `{ "recommendation": "grill" | "issue", "rationale": string }`.
